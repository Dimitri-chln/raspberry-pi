...
client.on("ready", async () => {
	...
	for (const commandFile of commandFiles) {
		const command =
			(require(Path.join(__dirname, "commands", commandFile)).default as DiscordBot.Command) ??
			(require(Path.join(__dirname, "commands", commandFile)) as DiscordBot.Command);
	...
	for (const autocompleteHandlerFile of autocompleteHandlerFiles) {
		const autocompleteHandler =
			(require(Path.join(__dirname, "autocomplete-handlers", autocompleteHandlerFile))
				.default as DiscordBot.AutocompleteHandler) ??
			(require(Path.join(
				__dirname,
				"autocomplete-handlers",
				autocompleteHandlerFile,
			)) as DiscordBot.AutocompleteHandler);
